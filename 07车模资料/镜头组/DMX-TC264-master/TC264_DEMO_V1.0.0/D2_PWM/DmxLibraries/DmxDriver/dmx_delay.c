...
{
    unsigned int stm_clk;
    stm_clk = IfxStm_getFrequency(IfxStm_getAddress((IfxStm_Index)stm));
    IfxStm_waitTicks(IfxStm_getAddress((IfxStm_Index)stm), (unsigned int)((unsigned long)stm_clk*time/1000000000));
}
...
{
    unsigned int stm_clk = IfxStm_getFrequency(IfxStm_getAddress((IfxStm_Index)stm));
    return (unsigned int)((unsigned long)(IfxStm_getLower(IfxStm_getAddress((IfxStm_Index)stm)) - systick_count[stm]) * 100000000 / stm_clk);
}
